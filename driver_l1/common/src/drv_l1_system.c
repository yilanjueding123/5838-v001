...
{
#if TV_DET_ENABLE
	system_clk_ext_XLAT_12M();		// ち场 XLAT 12MHz
#else
	...
	R_SYSTEM_LVR_CTRL = 0x20;				// 砞Θ 1.2V CorePower 1.32V ノ拈
...
INT32S sys_msdc_clk_restore(void)
{
	...
	#if 1//sensor_format == 1
	#if ((sensor_format == 1)||(INIT_MHZ == 96))
	MSDC_CLK = 96;
	#else
	MSDC_CLK = 144;
	#endif
