...
extern INT8U MODECTION_FLAG;
extern INT8U   card_insert_autorecord;
extern INT8U  modection_state_falg;
void state_video_record_entry(void *para, INT32U state)
{
	...
	INT16U md_check_tick, switch_flag;
...
//	msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_LED_SET, &led_type, sizeof(INT32U), MSG_PRI_NORMAL);
	...
	//if(ap_state_handling_storage_id_get()!=NO_STORAGE)
		...
		power_on_flag = 1;
		DBG_PRINT("xyxyxyxxyxyxyxyyxy\r\n");}
		//if(switch_state_get() == 0)
		//	msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
	} 
	
	else if((*(INT32U *) para == STATE_SETTING)|| (*(INT32U *) para == STATE_VIDEO_PREVIEW)|| (*(INT32U *) para == STATE_BROWSE)) {
		//if(*(INT32U *) para == STATE_BROWSE)
		//ap_video_capture_mode_switch(1,STATE_VIDEO_RECORD); 
		//else
...
//        		ap_music_update_icon_status();
			...
			case MSG_APQ_MOTION_DETECT_ACTIVE:
				...
				break;
#endif
            case MSG_APQ_VIDEO_RECORD_ACTIVE_MODE:
        	...
        	case MSG_APQ_VIDEO_RECORD_ACTIVE:
				...
				if(nRet == 0)
					...
					// 	}
					 ...
					 #if 0
					 if((ap_video_record_sts_get() & VIDEO_RECORD_BUSY)==0)//渾儂輛�蹁齣桭�儂ㄛ峈賸з煦望薹
					 	{
			...
			case MSG_APQ_POWER_ON_AUTO_RECORD:
				...
				if(power_on_flag /*|| card_insert_autorecord*/) {
					power_on_flag = 0;
