...
#define AVI_REC_MAX_BYTE_SIZE  0x7d000000//0x5dc00000//1500M//0x77500000// 0x1f400000//500mb//0x77500000// 0x70000000  //1879048192 Bytes//78643200//4B00000//0x77500000//1909mb
#define AP_STG_MAX_FILE_NUMS    625
#define BKGROUND_DETECT_INTERVAL  (15*128)  // 20 second
...
void ap_storage_service_freesize_check_switch(INT8U type)
{
	if (type == TRUE) {
		if (storage_freesize_timerid == 0xFF) {
			...
			sys_set_timer((void*)msgQSend, (void*)StorageServiceQ, MSG_STORAGE_SERVICE_FREESIZE_CHECK, storage_freesize_timerid, STORAGE_TIME_INTERVAL_FREESIZE);//STORAGE_TIME_INTERVAL_FREESIZE
...
extern  INT8U  card_insert_autorecord=0;
	  ...
	  if(card_first_dete)
				 {
				  ...
				  //if(card_first_dete == 2)		
				  //	{
				  //  card_insert_autorecord=1;
			...
			if (sd_upgrade_file_flag == 0) 
				{
				    ...
				    #endif
                     JH_Message_Get();
					//DBG_PRINT("set1:%d|set2:%d|set3:%d|set4:%d\r\n",display_osd_flag,power_on_record_flag,auto_poweroff_flag,record_led_flag);
					if(size >=1)
						{
						//WriteTool_card();
		...
		if(g_avi_index_9999_exist){
			...
			if(1)
			{
				...
				if(1)
				{
					...
					sprintf((char *)g_file_path, (const char *)"C:\\VIDEO\\MOVI%04d.avi", g_play_index);
				} else {
...
DBG_PRINT("File Path=%s\r\n", g_file_path);	
	...
	if (info_ptr->file_handle >= 0) {
		...
		if (!err_flag) {
			...
			g_err_cnt++;
			if (g_err_cnt <= g_avi_index/*g_file_num*/) {
				info_ptr->err_flag = STOR_SERV_OPEN_OK;
			} else {
	...
	info_ptr->total_file_number = g_avi_index;//g_file_num - g_same_index_num;
	msgQSend(ApQ, MSG_STORAGE_SERVICE_BROWSE_REPLY, info_ptr, sizeof(STOR_SERV_PLAYINFO), MSG_PRI_NORMAL);
}
