...
//extern void Time_card_storage(void);
...
void ap_state_config_initial(INT32S status)
{
	...
	if (status == STATUS_FAIL) { /* check nvdb initial status */
		...
		{
			 t_rtc time;
			if(Time_card_storage() ==STATUS_FAIL)
				{
...
void ap_state_config_default_set(void)
{
	...
	ap_state_config_video_resolution_set(0);	//720P
	...
	ap_state_config_pic_size_set(5);
	...
	ap_state_config_volume_set(8);

#if TV_DET_ENABLE
	ap_state_config_tv_out_set(0);//TV OUT: NTSC
#endif
	//ap_state_config_record_time_set(0);//循环录像的时间，设置为0表示关闭循环录像功能
