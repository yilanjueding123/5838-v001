...
void ap_peripheral_init(void)
{
	...
	for(i = IO_A2; i< IO_A6; i++)
	{
		gpio_init_io(i, GPIO_INPUT);
		gpio_set_port_attribute(i, ATTRIBUTE_LOW);
		gpio_write_io(i, 0);					//pull low
	}
	gpio_init_io(IO_E4, GPIO_INPUT);
	gpio_set_port_attribute(IO_E4, ATTRIBUTE_LOW);
	gpio_write_io(IO_E4, 0);					//pull low
	...
	gpio_write_io(AV_IN_DET, 0);		//pull high or low
	...
	for (i = 0; i < 5; i++)
	{
		if(0) //(gpio_read_io(AV_IN_DET) == !TV_DET_ACTIVE)
		{
			DBG_PRINT("jdjfnghg+++=\r\n");
			break;
		}
...
void ap_peripheral_key_init(void)
{
	...
	//gpio_init_io(FD_HD_PIN, GPIO_INPUT);		
	//gpio_set_port_attribute(FD_HD_PIN, ATTRIBUTE_LOW);	
	//gpio_write_io(FD_HD_PIN, 1);
	//	switch_key_state=gpio_read_io(SWITCH_KEY);
	//fd_hd_state=gpio_read_io(FD_HD_PIN);
	//fd_hd_prev_state=fd_hd_state;
}
...
void ap_peripheral_key_register(INT8U type)
{
	...
	if (type == GENERAL_KEY)
	{
		...
		key_map[0].key_function = (KEYFUNC)ap_peripheral_video_prev_exe;
...
void ap_peripheral_key_judge(void)
{
	...
	INT16U			key_down = 0;
	static INT8U	hd_fhd_cnt = 0, high_level_cnt = 0, video_stop_flg = 0;
	...
	for (i = 0; i < USE_IOKEY_NO; i++)
	{
		if (key_map[i].key_io)
		{
			...
			else 
			{
				...
				if (key_map[i].key_io == IO_B1)//PW_KEY)
				{
					power_keyup = 0;
				}
	...
	if (!auto_off_force_disable && !s_usbd_pin && ! (usb_state_get()) && auto_poweroff_flag)
	{
		if (power_off_cnt)
		{
			power_off_cnt--;
